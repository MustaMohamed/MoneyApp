...
## 6. The change against last month is the one figure coloured by direction

After last month's Out the caption prints this month's Out against the month before's full Out, not pro-rated, as a whole percentage with an arrow: `4 days left · Aug 7,400 ↑ 137%`. Every month compares with the month before it, past months included. It takes `deltaDisplay('expense', …)`'s vocabulary, the dashboard card's: up is danger, down is success, 0% a muted flat arrow. That makes it an exception to the money-colour ADR (2026-08-27), ruled 2026-09-26: the colour reads the direction of spend, not the sign of a balance. A screen reader hears it as one sentence naming the month before in full: `Spent <n>% more than <Month>`, `Spent <n>% less than <Month>` or `Spent the same as <Month>`.
