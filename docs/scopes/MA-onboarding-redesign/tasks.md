...
|---|---|---|---|---|---|
| MA-001 | Colour families, geometry tokens, and the 32-colour palette | awaiting-human | none | `feat/MA-001-color-and-geometry-tokens` | [#194](https://github.com/MustaMohamed/MoneyApp/pull/194) |
...
- **MA-007 now carries the verified parser behaviour, and it contradicts the spec.** `spec.md` states the current parser "rejects `5,000` → NaN". It does not. `5,000` passes validation and persists as **5**, as do `5,000.50`, `0x10`, `1e3` and `1_000` at wrong values — measured, not read. With business rule 6 setting current balance from opening balance, this is silent thousandfold corruption of the first number a user types, in both entry points, today. MA-007 carries the full case table so the fix is not tested against the milder description. **Corrected 2026-08-06 by @sarah** — `spec.md` § *Financial Logic — Account form* now carries the measured behaviour, so the spec and this task agree. The quoted sentence no longer exists in the spec; it is reproduced here only as the record of what was wrong.
