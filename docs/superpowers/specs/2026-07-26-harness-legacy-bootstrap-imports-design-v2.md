...
- **Date:** 2026-07-26
- **Status:** Approved, including the evidence-transparent legacy migration rule
...
That equality remains strict for every new receipt-bearing event. An exact
receipt-less event preserved at the repository migration anchor may cross a
recorded **evidence bridge** before its first range or between later ranges.
The bridge is transparent only when a manifest-bound canonical migration
artifact proves that the later endpoint descends from the earlier endpoint and
that the intervening diff contains only reserved workflow-evidence paths.
...
- For new receipt-bearing events, the first range starts at the form's exact
  checkpoint and every later `startHead === previous.endHead`.
- For an exact receipt-less event at the migration anchor, a non-equal first or
  later start is accepted only through a matching canonical evidence bridge.
- Every evidence bridge proves descendant ordering, exact canonical changed
  paths, evidence-prefix-only scope, and a deterministic digest.
- A bridge can never contain product code, harness implementation, generated
  delivery output, or any other non-evidence path.
...
- A receipt-less event with non-equal checkpoints additionally requires every
  gap to match a canonical evidence bridge in the manifest-bound legacy bridge
  artifact. When both bridge endpoints are available, replay recomputes its
  ancestry and path digest; when neither is available, replay verifies the
  portable bridge. Partial endpoint availability fails closed.
...
5. For receipt-less events, require exact byte presence at the migration anchor
   and resolve every non-equal checkpoint through the canonical legacy bridge
   artifact before applying the pure invariants.
...
- add a versioned canonical portable attestation to new nonempty bootstrap
  events, one manifest-owned legacy migration anchor, and one content-hashed
  legacy evidence-bridge artifact;
...
- exact anchored receipt-less history whose first and intermediate gaps are
  proven evidence-only, plus rejection for missing, changed, non-descendant,
  non-evidence, reordered, or partially available bridge evidence;
