...
The user says what they want. Two conversations later, every task exists on the board in one standard shape, ordered, with real dependencies, and a task whose dependencies are closed is pullable by an agent with no further input. Nothing about the work lives on disk.
...
| parent task | a task the user chose to break down further; created at Todo, never pulled | `/tickets` | `/tickets <n>` again |
...
- A parent closes when its last child closes, epic or parent task alike. No branch, no PR, no `Closes`.
...
| Status | Option id | Parent (epic, parent task) | Leaf | Set by |
|---|---|---|---|---|
| Todo | `f75ad846` | created, not brainstormed | never; a leaf is born Defined | `/epic`, `/tickets` for a child marked for its own breakdown |
...
Priority is the board's row order within a column. No Priority or Iteration field is added.
...
| | |
|---|---|
| Input | a parent: an epic number, or the number of a task created at Todo for its own breakdown |
...
1. Read the parent and map the code. One read-only scout maps the modules the Building list (or the task's Task Definition) touches, so cuts follow real seams.
2. Stop 1, the split. For each strategy in §7 that fits, one candidate table: task titles, edges, how many run in parallel, longest chain, and any task proposed for its own later breakdown. Recommended option first with the reason; a strategy that does not fit gets one line saying why. The user chooses.
...
5. Stop 2, the gate. Present the ordered table (ID, title, depends-on, and which are to be broken down later) and every body. Ask "Create these N tickets?". A rejected list costs nothing on GitHub.
...
Rewrite mode. `/tickets <parent> --rewrite` reads the existing children instead of drafting new ones, rewrites each body into the §6 standard keeping its number and title, and re-runs steps 4 to 6 with "Update these N tickets?" at stop 2 and no creation. This is the migration path for #378.
...
#378 is locked with nine children in the old three-line shape, all at Todo. Steps, after the skills exist:

1. Add `## Links` to #378's body (mockup `none` until MA-014) and set #378 to Defined.
2. `/tickets 378 --rewrite`: bodies to §6, depends-on kept, reviewer audit, "Update these N tickets?".
...
| 2026-09-02 | No size field; a task the user chooses to break down further is created at Todo and re-enters `/tickets` |
| 2026-09-02 | Three cuts with no preference order; the user chooses from proposed options |
| 2026-09-02 | Splitting is recursive; `/tickets` runs on an epic or on a task created at Todo for its own breakdown |
