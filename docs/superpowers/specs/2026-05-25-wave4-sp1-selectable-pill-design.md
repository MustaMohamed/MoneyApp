...
- **`#888` routed to `CoreTokens.text2` (per the user's "fix all" directive).** Initially preserved for zero-visual-change, then tokenized when the user asked to fix every review finding — `#888` is a hardcoded hex that violates the no-hardcode rule. Replaced in all three filter accordions (`account`/`category`/`amount`): the chevron `color` and the dot-fallback (`x.color ?? CoreTokens.text2`). All three sibling chevrons changed together to avoid a 2-of-3 inconsistency in the same sheet. **Minor visible delta:** the muted gray `#888` becomes the muted blue-gray `CoreTokens.text2` (`#6B7F99`) on the accordion chevrons and on the dot of any color-less category/account — confirm at device QA. `amount_accordion.tsx` is otherwise out of SP-1 scope (its EGP/USD toggle is SP-4); only its 1-line chevron color was touched.
