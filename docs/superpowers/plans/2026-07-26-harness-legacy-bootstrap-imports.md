...
**Architecture:** A pure bootstrap module validates graph membership, checkpoints, chain order, dependencies, checks, and replacement monotonicity. The Git scope layer attests commit ancestry and exact diffs and emits a canonical portable receipt; CLI append and repository replay share those contracts. A manifest-owned migration anchor permits only exact pre-change receipt-less events, while projection preserves completed work across graph replacement.
...
## Task 5: Compatibility and integration gates
...
1. the existing Phase 3 receipt-less event chain anchored at the migration
   boundary with feature commits unavailable;
2. a generic onboarding-shaped activation with pre-harness asset commits;
3. replayed disjoint, invented, stale, and reordered completions;
4. replacement attempts that omit or reopen completed work.

Run:

```bash
node --test scripts/harness/__tests__/task_integration.test.js
```

Expected: FAIL until all integration surfaces use the new policy.

- [ ] **Step 2: Apply the smallest integration corrections**

Adjust only shared context plumbing, manifest fixture construction, and
assertions needed by the failing end-to-end cases. Do not add an initiative ID,
onboarding condition, provider call, or command auto-execution.

- [ ] **Step 3: Run all task tests and commit**

```bash
node --test scripts/harness/__tests__/task_*.test.js
git add scripts/harness/__tests__/task_integration.test.js \
  scripts/harness/__tests__/task_store.test.js \
  scripts/harness/__tests__/task_cli.test.js
git commit -m "test: lock bootstrap migration invariants"
```
