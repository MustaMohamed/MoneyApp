# Audit Remediation Backlog — Linear Issue Drafts

Source: [2026-07-29-full-technical-audit.md](../reviews/2026-07-29-full-technical-audit.md) (89 findings, verifier-corrected). One issue per wave; create in Linear in this order — dependencies are noted. Wave 1 of the original sequencing (verify-pr green) already shipped via PR #175/#177.

---

## Issue 1 — Fix commitment payment lifecycle

**Labels:** `bug`, `financial-correctness` · **Effort:** L · **Blocked by:** —
...
## Issue 2 — Category deletion integrity

**Labels:** `bug`, `crash` · **Effort:** M · **Blocked by:** —
...
## Issue 3 — Money display fixes (after formatter consolidation)

**Labels:** `bug`, `financial-correctness` · **Effort:** M · **Blocked by:** Issue 4
...
## Issue 4 — Consolidate money formatting layer

**Labels:** `refactor` · **Effort:** M · **Blocked by:** — (do BEFORE Issue 3 or 13 sites get touched twice)
...
## Issue 5 — Budget envelope identity

**Labels:** `bug`, `financial-correctness` · **Effort:** M · **Blocked by:** —
...
## Issue 6 — Cairo Nights typography actually renders

**Labels:** `bug`, `design-system` · **Effort:** S · **Blocked by:** —
...
## Issue 7 — Bounded render + query performance

**Labels:** `performance` · **Effort:** M · **Blocked by:** Issues 1–5 (correctness first)

Top wins, in order: gate sheet children behind `hasEverOpened` in `components/ui/sheet.tsx` (**H7** — one file, fixes Budget's 10-sheet eager mount app-wide); staleness gates on Budget/Commitments focus + stop Dashboard invalidate-on-blur (M13/M32/L26); hoist `formatAmount`'s `Intl` (M24 — may land with Issue 4); index-friendly predicates in `budget_stats.ts` + `transactions.ts` (L2/L34); lazy swipeable action tiles (L16); move startup housekeeping off first paint (L23).

## Issue 8 — Real coverage gate + test-suite repairs

**Labels:** `testing` · **Effort:** L · **Blocked by:** Issues 1–5 (never instrument coverage over known-broken logic)
...
## Issue 9 — Module boundaries + legacy retirement

**Labels:** `refactor` · **Effort:** L · **Blocked by:** Issue 8 (coverage guards the refactor)
...
**Not scheduled (user decisions):** goals tab stub visibility (L18) · render-test policy (M36) · `ON DELETE` schema migration (Issue 2 follow-up).
