...
The board rules the define and delivery skills apply, moved out of `CLAUDE.md` so they load when a skill needs them. `scripts/board_next.mjs` encodes the transition table below; `__tests__/scripts/board_next.test.ts` pins it.

Work is defined on GitHub and delivered from GitHub. The issue is the record; `.work/<MA-id>/` and `~/.ship/MoneyApp/` hold transient working files.
