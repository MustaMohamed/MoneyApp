...
export function CommitmentAccountAccordion({
  accounts,
  selectedIds,
  expanded,
  onToggleSection,
  ...
  const summary = formatCommitmentSelectionSummary(
    selectedNames,
            ...
            <View className="mt-3 flex-row flex-wrap gap-1.5">
              {accounts.map((account) => (
                <SelectablePill
                  ...
                  onPress={() => onToggleId(account.id)}
                  startIcon={
                    <MaterialCommunityIcons
                      name={TYPE_OPTIONS.find((o) => o.type === account.type)?.icon ?? 'bank'}
                      size={ms(13)}
