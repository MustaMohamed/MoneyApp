 ...
 * `--shadow-surface` token; the `elevation`/`shadowOpacity` pair does not
 * beat it either, because uniwind keys that token as `boxShadow`, a separate
 * RN pipeline. With `userInterfaceStyle: "automatic"` (`app.json:8`) and
 * `--surface-shadow` undeclared, an OS-light device paints HeroUI's real
 * light shadow — `boxShadow: 'none'` is the deterministic both-variant kill,
 * device-proven.
 */
