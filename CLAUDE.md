...
Path-scoped rules in `.claude/rules/` load automatically when working with matching files: `database.md` (queries, migrations, repositories), `ui.md` (all `.tsx`, styling, HeroUI, sheets), `state.md` (stores, state, hooks), `money.md` (domain resolvers, rounding, formatting), `tests.md` (everything in `__tests__/`), `review.md` (the five recurring defect classes, all of `src/**`). Project skills: `heroui-native` (UI catalog + patterns), `money-rules` (financial contracts), `moneyapp-testing` (test patterns), `device-qa` (QA matrices), `emulator-verify` (drive the app on the emulator yourself), `epic` · `boundaries` · `tickets` · `issue-review` (the define workflow, see *Workflow*), `prep` · `ship` (delivery, see *Workflow*), `board` (read the board: next action per ticket, dependency graph), `queue` (work the board: one local task session per ticket to prep or ship, three in flight), `moneyapp-expert-panel` (inline personas), `unslop` (the output contract — mandatory for all composed output; see *Answering me*).
...
Returns 0 today; it must not grow.
...
`/board [n] [graph|text]` reads it: every open ticket with the skill or command to run next, grouped by who acts, and the dependency graph as an inline widget when the shape needs it. A plain poll needs no session: `bash scripts/board.sh next [n]` prints the same text report from a terminal in about five seconds, `--json` for a script, and `bash scripts/board.sh serve` opens the same report as a local page. `scripts/board_next.mjs` carries the rules, one per row of the table below, and writes nothing. `/queue [n] [k]` works it: for each `/prep` or `/ship` row at the top of the ranking it starts a local task session holding that one command, `k` in flight at most (default 3, one per emulator slot); each ticket runs in its own sidebar session and its gates are answered there. Define work is never started by the queue.
...
**Emulator verification** runs on tickets whose header line says `Verify emulator`, anything that changes what a screen shows or what the app writes. `/ship`'s implementer runs it as the render pass and the battery's render lens runs it again, independently; the `emulator-verify` skill carries the mechanics. It is a second net under the same defects: **device QA is unchanged**, on real hardware, and typography, shadows, gesture feel and performance are visible nowhere else.

Gotcha: **device QA does not run in the worktree.** A worktree whose `node_modules` is a symlink passes `tsc`, `jest`, and lint but breaks device builds; expo-router resolves zero routes. Check the PR branch out in the primary repo for device QA. Emulator verification *does* run in the worktree once it has a real `node_modules` (`/prep`'s APFS clone, or `npm ci`). **Three emulators run in parallel, one ticket each, and `mqa claim` is the first call of any run**: it takes a device for this worktree and prints the Metro port to start. Without a claim `mqa` refuses as soon as a second device is up, rather than driving another session's. `mqa release` when the ticket is done, `mqa claims` to see who holds what.

**A Gradle build is not part of that price by default.** Ask `mqa needs-build`: only a native-surface change rebuilds, most task diffs are JS-only, and the branch under test reaches the device over Metro either way. Run the parity chain *before* building, so the render pass and the render lens share one APK instead of each making their own. And scope the walk: **if a unit test can assert it, the emulator must not.** All of this is in the `emulator-verify` skill, with the measurement behind it.
...
- **Never** push a bare `(tabs)` href from a screen reachable from `/stacked` — a `(tabs)` target needs a stacked twin route and `stackedPrefixOf(usePathname())`; bare, it drops the user back into `(tabs)`, mounting a second tab bar and sending Back to the Dashboard. A target already on the `(app)` Stack (`/accounts/*`, `/settings/*`) has no twin to prefix and stays bare. View commitment on the transaction detail pushes the stacked twin from both copies, so the commitment detail sits above the tabs and Save or Deactivate return to the transaction (MA-072).
