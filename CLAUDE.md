...
| Ready For Development | Defined | `/issue-review` when an ask is open or a Depends on names an open issue again; `/tickets --rewrite` on a rewritten body, `Reviewed` back to `none` | the review no longer stands |
