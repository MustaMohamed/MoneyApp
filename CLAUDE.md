...
**Delivering a ticket is two skills that compose on the board.** `/plan <n>` takes a leaf task from Ready For Development to Planned: it creates the ticket branch linked to the issue (`gh issue develop`), a cold planner writes `docs/plans/MA-XXX.md`, a fresh reviewer checks it against Acceptance and the code, and the plan is committed on the branch. `/ship <n>` takes it from Planned to Done: implement, review battery, triage and fix, re-check, merge; on a Ready For Development ticket it runs `/plan` first, and `/ship` alone pulls the top Planned row. The plan never reaches main: ship removes it in its last commit before the merge. One human gate, the merge; every destructive repository operation is an explicit request from me.
...
Gotcha: **device QA does not run in the worktree.** A worktree whose `node_modules` is a symlink passes `tsc`, `jest`, and lint but breaks device builds; expo-router resolves zero routes. Check the PR branch out in the primary repo for device QA. Emulator verification *does* run in the worktree once it has a real `node_modules` (`/plan`'s APFS clone, or `npm ci`); give the worktree its own Metro port, because `adb reverse` is global per device and sharing 8081 silently serves the primary repo's bundle.
