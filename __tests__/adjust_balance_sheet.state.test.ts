...
describe('useAdjustBalanceSheetState initial state', () => {
  it('starts with empty input and error', () => {
    const s = useAdjustBalanceSheetState.getState().state;
    expect(s.input).toBe('');
    expect(s.error).toBe('');
  });
});
...
describe('useAdjustBalanceSheetState setters', () => {
  it('setInput updates input value', () => {
    ...
    expect(useAdjustBalanceSheetState.getState().state.input).toBe('123.45');
  });
  ...
  it('setError updates error value', () => {
    ...
    expect(useAdjustBalanceSheetState.getState().state.error).toBe('invalid');
  });
...
describe('useAdjustBalanceSheetState initialize', () => {
  ...
  it('handles zero balance', () => {
    ...
    expect(useAdjustBalanceSheetState.getState().state.input).toBe('0');
  });
  ...
  it('handles negative balance', () => {
    ...
    expect(useAdjustBalanceSheetState.getState().state.input).toBe('-50');
  });
});

describe('useAdjustBalanceSheetState reset', () => {
  it('returns to defaults', () => {
    useAdjustBalanceSheetState.setState({
      state: { input: '999', error: 'something' },
    });
