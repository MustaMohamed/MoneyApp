...
describe('useDecimalInputState', () => {
  ...
  it('does not reset text when a hook instance rerenders with a new initialText', () => {
    const { result, rerender } = renderHook(
      ({ initialText }: { initialText: string }) => useDecimalInputState(initialText),
      {
        initialProps: { initialText: '1' },
      },
    );

    act(() => result.current.setText('11'));
    rerender({ initialText: '2' });

    expect(result.current.state.text.value).toBe('11');
  });
});
