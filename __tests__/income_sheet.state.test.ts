...
describe('useIncomeSheetState', () => {
  ...
  // `String(1e-7)` is '1e-7', which `DECIMAL_PATTERN` rejects, leaving an unsaveable field.
  ...
  // The suggestion averages `egp_amount`, which has no CHECK, so sub-cent values reach here.
