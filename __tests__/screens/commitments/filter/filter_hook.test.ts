...
describe('useCommitmentFilterSheet', () => {
  it('disables Apply when draft and applied filters match', () => {
    const { result } = renderHook(() => useCommitmentFilterSheet());

    expect(result.current.state.canApply).toBe(false);
  });

  it('enables Apply when amount type or recurrence filters change', () => {
    const { result } = renderHook(() => useCommitmentFilterSheet());

    act(() => result.current.toggleAmountType(AmountType.Variable));
    expect(result.current.state.draftCount).toBe(1);
    expect(result.current.state.canApply).toBe(true);

    act(() => result.current.toggleRecurrencePreset(RecurrencePreset.Monthly));
  ...
  it('tracks amount currency only when amount range is active', () => {
    const { result } = renderHook(() => useCommitmentFilterSheet());
