...
describe('resolveCommitmentDetailViewState', () => {
  const cases: [string, CommitmentDetailViewStateInput, CommitmentDetailViewState][] = [
    [
      ...
      { hasCommitment: false, status: 'loading', hasRows: false, refreshError: false },
      'notFound',
    ],
    [
      'not found beats a first-load error',
      { hasCommitment: false, status: 'firstLoadError', hasRows: false, refreshError: false },
      'notFound',
    ],
    [
      'not found beats a refresh error over rows',
      { hasCommitment: false, status: 'ready', hasRows: true, refreshError: true },
      'notFound',
    ],
    [
      'a cold load reads loading',
      { hasCommitment: true, status: 'loading', hasRows: false, refreshError: false },
      'loading',
    ],
    [
      'a warm load reads ready',
      { hasCommitment: true, status: 'loading', hasRows: true, refreshError: false },
      'ready',
    ],
    [
      'a cold failure reads firstLoadError',
      { hasCommitment: true, status: 'firstLoadError', hasRows: false, refreshError: false },
      'firstLoadError',
    ],
    [
      'a first-load error with rows reads refreshErrorWithData',
      { hasCommitment: true, status: 'firstLoadError', hasRows: true, refreshError: false },
      'refreshErrorWithData',
    ],
    [
      'a warm failure reads refreshErrorWithData',
      { hasCommitment: true, status: 'ready', hasRows: true, refreshError: true },
      'refreshErrorWithData',
    ],
    [
      'a zero-row success reads ready',
      { hasCommitment: true, status: 'ready', hasRows: false, refreshError: false },
      'ready',
    ],
    [
      'a success with rows reads ready',
      { hasCommitment: true, status: 'ready', hasRows: true, refreshError: false },
      'ready',
    ],
  ];
