...
describe('resolveNetWorthForeignCaption — net_worth_breakdown_sheet.tsx:164', () => {
  it('shows cents on a non-whole USD net worth — base: 1,251, head: 1,250.75', () => {
    expect(resolveNetWorthForeignCaption(1250.75)).toBe('≈ 1,250.75 USD');
  });

  it('shows the row-12 whole-number shape — base: 100, head: 100.00', () => {
    expect(resolveNetWorthForeignCaption(100)).toBe('≈ 100.00 USD');
  });
  ...
  it('renders the absent-rate placeholder unchanged when netWorthForeign is undefined', () => {
    ...
    // a rate.
    expect(resolveNetWorthForeignCaption(undefined)).toBe('— USD');
  });
});
