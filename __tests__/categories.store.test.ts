...
describe('useCategoriesScreenStore', () => {
  ...
  it('starts with linkedCount at 0', () => {
    const { result } = renderHook(() => useCategoriesScreenStore());
  ...
  it('setLinkedCount updates linkedCount and accepts 0', () => {
    const { result } = renderHook(() => useCategoriesScreenStore());

    act(() => result.current.setLinkedCount(47));
    expect(result.current.state.linkedCount.value).toBe(47);

    act(() => result.current.setLinkedCount(0));
    expect(result.current.state.linkedCount.value).toBe(0);
  });

  it('reset clears all transient category data', () => {
    const { result } = renderHook(() => useCategoriesScreenStore());
