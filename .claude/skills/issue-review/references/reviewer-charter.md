...
- **S3b Family.** An Out of scope line that excludes another site of the ticket's own defect (the same check, resolver, slot or string on another screen or module) is a delta: the site moves into Acceptance, or the ticket is rewritten as the class ticket with every site from one `git grep` and the Goal naming the defect. One ticket per class, never one per site; cite the sibling that already shows the pattern when one exists.
...
- **P3 Size.** Count per `.claude/skills/tickets/references/splitting.md` § Size gate: the map's file list, or your own when there is no map, every file a Rule or an Acceptance line implies included, with a line estimate per file from the code as it is; one outcome, or a bundle under § Floor. Over the gate, or at it, is an `ask` delta proposing the seam; the gate is hard, so the ask offers seams or Todo for its own `/tickets` run, never keeping the body whole. Under ~100 lines with an open ticket on the milestone that changes the same screen or fixes the same defect family, and room in that ticket's gate: an `ask` delta proposing the bundle per § Floor. A body whose `Size:` line disagrees with the count, or lacks one, gets a mechanical delta with the counted list. Never take the body's figure as the count. A body at Todo was chosen by the user for its own breakdown and is exempt.
