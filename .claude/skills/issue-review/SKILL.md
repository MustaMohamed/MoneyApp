...
7. **Apply.** Per issue with accepted deltas, take the body fetched in step 1, apply the deltas, `gh issue edit <n> --body "$BODY"`. The lock line and the title stay. A parent body, an epic or a split leaf, is edited here only while it has no children; once children exist the review edits them.
8. **Mark and promote.** A leaf passes when every delta on it was applied or rejected by the user and no `ask` is open; an issue with an open `ask` stays as it was, `Reviewed none`, Defined. Per passing leaf under review, a task with no sub-issues: write `Reviewed <today>` on its header line, `gh issue edit <n> --body "$BODY"`. A ticket at Ready For Development whose Depends on now names an open issue goes back, `bash scripts/board.sh status <n> Defined`. Then once, `bash scripts/board.sh promote <n>`, which moves every marked leaf with every Depends on closed to Ready For Development: the children when `<n>` has any, `<n>` itself otherwise. A parent, epic or split task, gets no mark and stays at Defined. Nothing is created or closed.
