...
- **Parent**, any issue with a parent: the parent's Rules carried, a Building bullet served, the size gate counted against the real files, Verify and Flags true to the code, Depends on real.
...
2. **Map the code.** Reviewing children: one read-only scout (`subagent_type: Explore`, breadth "medium") lists, per child, the folders, screens, migrations, money paths, native config and user copy it touches, and names its main surface, the deepest folder under `src/modules/` it changes; a child with no code surface (a design ticket) is its own surface. The scout also builds each child's file list per [splitting.md § Size gate](../tickets/references/splitting.md), every file a Rule or an Acceptance line implies included, with a line estimate per file, beside the body's own `Size:` line. Reviewing one issue: no scout; the reviewer maps and counts for itself.
