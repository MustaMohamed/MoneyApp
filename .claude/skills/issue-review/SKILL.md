...
7. **Apply.** Per issue with accepted deltas, take the body fetched in step 1, apply the deltas, `gh issue edit <n> --body "$BODY"`. The lock line and the title stay. A parent body, an epic or a split leaf, is edited here only while it has no children; once children exist the review edits them. Then the two status corrections the header can force: a ticket at Ready For Development whose Depends on now names an open issue goes back, `bash scripts/board.sh status <n> Defined`; a header that lost its last open dependency is promoted, `bash scripts/board.sh promote <parent>`. Nothing is created or closed.
