...
4. Record the ledger in `task.md` (chunk slug, owned spec sections, exposed interface, status) and mirror status rows in `state.md` → `## Chunks`. Mark each chunk **disjoint** (parallelizable) or **dependent** (waits for named prerequisites' merges, or folds into the final PR). **Bias the cut toward disjoint** — every dependent edge adds one human-merge wait to the critical path, and merge latency, not agent time, dominates chunk-mode wall-clock.
