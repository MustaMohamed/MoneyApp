...
1. **Conventions:** check the diff against the repo's `CLAUDE.md` and the `.claude/rules/` files matching the touched paths (`ui.md`, `state.md`, `database.md`, `money.md`, `tests.md`): state management idiom, layering, naming, file placement, comment rules. Cite the rule-file line or sibling file in every convention finding.
   **Then `review.md`'s five defect classes, each answered explicitly against this diff** — silent async failure, focus-reload churn, money display/rounding, index-defeating SQL, derived state stored as durable state. The 2026-07-29 audit proved all five recur here; a class that does not apply gets one line saying so, not silence.
