...
2. Write the plan for a cold reader, as numbered steps. Each step names:
   ...
   - the test that proves it — written first where the repo's conventions test that layer (check `.claude/rules/tests.md` and `CLAUDE.md` for what gets tested — logic-only `.ts` tests in `__tests__/`; do not plan tests the repo forbids). Full cycles are integration tests in Jest against real SQLite — never plan an emulator scenario for behavior a test can assert; the emulator pass (spec §8) covers design conformance only.
