...
6. **Write the consolidated list to `findings/p8-cycle-<n>.md`** — each finding with `path:line`, quoted code, severity, and required outcome. This file is what the fixer receives and what P9 verifies against; a list that lives only in conductor context dies with the session.
7. **Human gate (only when needed):** spec-level defects, scope expansions, and disputes go to the human as one compact list — finding, consequence, recommendation. **While this gate is open, pre-stage the amendment** (spec delta + plan-charter draft) under `prestage/`; the answer releases execution.
