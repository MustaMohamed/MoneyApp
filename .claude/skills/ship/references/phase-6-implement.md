...
Do not dispatch `@layla`, the one `.claude/agents/` persona, as the implementer — she decides money rules and writes no code; /ship composes its own implementer. Assemble the implementer prompt from three layers at dispatch time:
