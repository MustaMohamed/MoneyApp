...
| hero, current month | A1, MA-098 | the walk's seed: this month with income and expenses, last month with expenses | `mqa ui` reads `Out this month`, the month's full name with no year (A1 `September`), the amount row as one node `<Out> EGP`, one node per column (`In <amount>`, `Net +<amount>`, `Left of income <n>%`), `<n>% of income spent` and `<n> days left · <Mmm> <amount>` followed by the change node, whose content-desc reads `Spent <n>% more than <Month>` or `less` once, n the half-up rounding of (this Out − last Out) × 100 / \|last Out\| over the `mqa db` sums, and the rail node's content-desc `Expenses are <n>% of income` once; Out and In equal the `mqa db` month sums under the ledger rule (an income on a card reduces Out, transfers and card payments count 0); one shot: In and Net in success, the rail in success filled to the share |
...
| hero, scoped from the sheet | A1, MA-098 | seed two bank accounts, each with two expenses this month of different sizes, beside other accounts with expenses; open the filter sheet, expand `Accounts`, select both, Apply | the title reads `Out this month`, and the amount node's Out equals `mqa db` `select sum(egp_amount) from transactions where account_id in ('<a>','<b>') and type='expense' and transaction_date between '<month start>' and '<month end>'`, not the all-accounts sum; the filter button reads `Filter, 1 active`; one shot |
...
| hero, empty month | A5, MA-098 | the month pill to a month with no rows whose month before has no rows either | the amount node reads `0 EGP` and the column nodes `In 0`, `Net 0` and `Left of income —`, no share caption, the caption `<Mmm> —` and no change node; no skeleton and no alert; one shot |
| hero, over income | no frame, MA-098 | seed an account with In 1,000 and Out 1,200 this month and open it through `See all` | the column nodes read `Net −200` and `Left of income −20%` (U+2212), the caption `120% of income spent`, the rail full in danger; one shot |
| hero, credits exceed expenses | no frame, MA-098 | the `row, card credit` seed: an income on a credit card with no expenses on that card this month, then `See all` on the card | In 0 and Out below 0 under the ledger rule (`REPORTING_SIGN_SQL.out`, `reporting_sign.ts:3-7`); the caption node reads `Credits exceed expenses`, the rail's content-desc reads it once, the rail is empty, the column node reads `Left of income —` and the amount node `−<amount> EGP` with U+2212; one shot |
...
| hero, change against last month | no frame, MA-098 | seed one account with Out 500 three months back, 2,000 two months back, 1,000 last month and 1,000 this month; open it through `See all` | this month: a muted `arrow-right`, `0%` and the change node's content-desc `Spent the same as <Month>`; the pill one back: a success `arrow-down`, `50%` and `Spent 50% less than <Month>`; the pill two back: a danger `arrow-up`, `300%` and `Spent 300% more than <Month>`; `<Month>` is the month before the one shown, in full; the caption row's bounds read the same in all three; three shots |
