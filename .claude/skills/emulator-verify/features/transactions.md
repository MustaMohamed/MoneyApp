...
| hero, empty month | A5, MA-098 | the month pill to a month with no rows | the amount node reads `0 EGP` and the column nodes `In 0`, `Net 0` and `Left of income —`, no share caption, the caption `<Mmm> —`; no skeleton and no alert; one shot |
