...
| section count badge | B1, MA-086 | seed push past a hundred active accounts, All selected. This screen draws one `Your accounts` header carrying the total, so it proves the three-digit count; the one- and two-digit counts are the per-type badges on `dashboard.md` | the badge label's `TextView` bounds ÷ 2.625 read `lineHeightFor(msFont(12))` = 16 ± 1 high, so the pill is 16 + 4 (`py-0.5`) = 20; the pill stays wider than it is tall (label width + 16 for `px-2`); one shot of the `Your accounts` header row, judged against B1 for the title, the action link and the header margins |
