...
Two states need a schema-020 database: `upgraded from 020`, and `edit sheet, migrated swatch selected`, whose `Walk Blue` must still hold `#185FA5` when 021 runs so it lands on the `#2381DF` swatch; a seed built at schema 21 leaves it on `#185FA5` and no swatch reads `selected`. Schema 020 is every migration up to 20 and `schema_migrations` rows 1 to 20, so the seeded categories and `Walk Blue` still hold the old hexes and the launch runs 021 over them. Every other MA-103 state needs only a database holding the rows below, at any schema, because a fresh install holds the same tones.
