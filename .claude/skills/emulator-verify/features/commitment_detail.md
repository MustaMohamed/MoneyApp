...
- The pill is a `View` with no touch handler, so it is flattened out of `ui.xml`: read the label `TextView` and add `py-0.5` (2 dp each side), or measure the painted pill on the shot (`README.md` § The rule).
