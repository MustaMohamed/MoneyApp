...
| validation, no account | D6, MA-105 | a fresh `add expense` (the FAB opens with no account), `$MQA fill 'id="amount-hero-value"' 5`, `$MQA tap 'label="Save"'` | `mqa bounds 'id="from-account-ring"'` matches the `from-account-row` bounds (43.81 high on `Pixel_2_API_34`); `mqa read` finds `Fix the 2 fields marked above.`, the Account and Category rings both drawn; one shot of the sheet |
