...
|---|---|---|---|
| add expense | D1, MA-110 | the FAB on the Transactions tab, one bank account seeded | `mqa read` greps `Add transaction` once; the four type tab triggers' bounds read 28 ± 1 high inside the 36 track, each with its glyph in its type colour; `from-account-row`, `category-row` and `date-row` bounds read 44 or more high; `transaction-form-fact-group` holds `category-row` and `date-row` 17 dp in from its edge (16 inset, 1 border) and `from-account-row` sits above it, flat; after one `mqa scroll down`, `note-row` and the Note `EditText` (`text="Add a note (optional)"`) read 44 or more high, the last row of the card with no hairline under it; the Save node reads 48 ± 1 high on a single accent fill; one shot of the sheet |
...
- The skeleton's containers are not in the tree: the root's `Loading transaction` label folds them, so `bounds` on the skeleton testIDs reads `no match`. Prove the loading state from the shot.
