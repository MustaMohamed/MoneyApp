...
7. **Size.** Do not take the header figure. Count the distinct `File:` paths outside `__tests__/` and the steps: more than 12 files or more than 8 steps is a finding, "sized past one PR", whatever the header says. Then build your own line estimate from the files you opened in step 0; past ~400 outside tests is the same finding. The ticket goes back for a cut, it does not get a bigger plan. On the accounts redesign the header ran a third of the delivered size on eight tickets, and one plan at ~540 was reviewed through.
