...
- **Size.** Count before you estimate: the files your steps name outside `__tests__/` and generated code, and the steps themselves. More than 12 files, more than 8 steps, or two product outcomes in one ticket: return one gap, "sized past one PR", with the seam you would cut at, and do not plan it. Then estimate the changed lines per file you open, and sum; past ~400 is the same gap. Eight plans on the accounts redesign estimated a third of what shipped (MA-039: ~410 planned, 1,494 delivered; MA-052: ~220, 803), so a figure you did not build per file is wrong by that factor.
