...
The rules are the same at every level; only the parent changes. `/tickets <task>` cuts a task into sub-issues with the next MA numbers, at Defined, and moves that task to Ready For Development as a parent. The task is either one created at Todo for its own breakdown or a leaf at Defined or Ready For Development that turned out bigger than one PR; the leaf keeps its body and its children's Acceptance covers it line by line. A parent closes when its last child closes and is never planned.
