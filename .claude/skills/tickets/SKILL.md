...
- Defined, body starting with `Scope locked`: an epic.
- Todo: a task created for its own breakdown. A task at Todo meant to stay one leaf goes to `/boundaries` instead.
- Defined or Ready For Development, body in the ticket standard, no sub-issues (`gh api repos/MustaMohamed/MoneyApp/issues/<n>/sub_issues --jq length` prints 0): a leaf that turned out bigger than one PR, cut further here as it is, with no reset to Todo.

Planned or later: a branch exists and the ticket is in delivery; say so and stop. Children already exist: `--rewrite` here or `/issue-review <n>`. Anything else: say what you found and stop. `--rewrite` needs existing children: `gh api repos/MustaMohamed/MoneyApp/issues/<n>/sub_issues --jq '.[].number'`.
