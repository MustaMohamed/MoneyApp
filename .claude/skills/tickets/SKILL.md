...
5. **Create**, per ticket, in order:
   ...
   `--rewrite`: `gh issue edit <child> --body "$BODY"` keeps number, title and board Status, and touches only a child at Todo, Defined or Ready For Development; a child past that is in delivery, listed as skipped, and left alone.
