---
description: Run one task through steps 4-9 and stop at its PR
---

@sarah Run **one task** through steps 4–9 of the ten-step workflow. The steps, gates, status vocabulary, retry caps, and git rules are in your agent file and in CLAUDE.md — follow them as written. This is `/scope` narrowed to a single task: same reviewers, same four review gates, same critical triggers, same status rules — and status is the `status:*` label on the task's GitHub issue, never a field in the repo.
