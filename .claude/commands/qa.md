...
4. Wait for my per-item results, then record them under `## Device QA` in the ticket's `task.md` (`~/.ship/MoneyApp/<ticket>/task.md`), using the skill's template, and state the verdict: pass, or fail with the items that route back through the review battery's fix loop (ship phase 8).
