...
- **Changing a resolver changes every preview that calls it.** 11 files import these; before altering a signature or a rounding point, check `grep -rl resolveTransactionAmounts src`. The iron rule elsewhere in the app — *displayed money comes from the same function that performs the write* — only holds if these stay the single derivation point.
