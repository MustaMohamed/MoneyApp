...
- **A sheet with a `footer` takes `liftsAboveKeyboard` on `components/ui/sheet.tsx`, not `keyboardBehavior="extend"`.** `extend` reaches the largest snap point and stops; it never lifts the absolute footer, whose `translateY` subtracts a keyboard height that `android_keyboardInputMode="adjustResize"` pins to zero (`BottomSheet.tsx:822-828`, `BottomSheetFooterContainer.tsx:29`). The prop flips that input mode to `adjustPan` so gorhom compensates for the keyboard itself. Android only — iOS never reaches the guard.
