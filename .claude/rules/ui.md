...
- HeroUI `Card` and `Dialog.Content` ship **no border, ever, by default**, and a custom shadow token `shadow-none` cannot override; see the Card = Surface trap in the `heroui-native` skill before migrating a `View` to `Card`.
