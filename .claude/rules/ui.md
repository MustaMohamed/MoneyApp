...
- **CTA:** `Size.ctaHeight` (52) · `Radius.cta` (13) · gold on midnight-blue text. Redesigned screens (onboarding, so far) render it flat `--accent` via `Button`'s `flat` prop; legacy screens keep the two-stop gradient until the redesign reaches them (spec.md § Known design/codebase disagreements, item 1, update 2026-09-01). `EmptyState`'s CTA is flat on every caller, all nine screens, not per-screen opt-in (MA-025, ruling 2026-09-07).
