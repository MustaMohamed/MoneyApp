...
- Keyboard-aware inputs: `useBottomSheetAwareHandlers()` on `onFocus`/`onBlur` + `keyboardBehavior="extend"` on `Content`. **Call the hook from a component rendered inside the sheet's `children`** — above `<Sheet>`, or in a `.hook.ts` consumed by the component that renders `<Sheet>`, it is outside the gorhom context and the handlers are silent no-ops.
- **A sheet with a `footer` takes `liftsAboveKeyboard` on `components/ui/sheet.tsx`, not `keyboardBehavior="extend"`.** `extend` reaches the largest snap point and stops (`BottomSheet.tsx:822-828`, the `extend` guard), and never lifts the absolute footer, whose `translateY` subtracts a keyboard height (`BottomSheetFooterContainer.tsx:29`) that `android_keyboardInputMode="adjustResize"` pins to zero — `heightWithinContainer = 0` at `node_modules/@gorhom/bottom-sheet/src/components/bottomSheet/BottomSheet.tsx:1696`, inside the `OnKeyboardStateChange` reaction, with the matching guard that skips the `interactive` compensation at `:846-855`. The prop flips that input mode to `adjustPan` so gorhom compensates for the keyboard itself. Android only — iOS never reaches the guard.
...
- **A style object's `fontSize` override always pairs `lineHeight: lineHeightFor(fontSize)`** (`theme.ts:111`) — otherwise the className's line-height keeps the old size and drifts from it. `moneyapp/font-size-pairs-line-height` enforces this at error severity, argument included: `lineHeightFor(Type.body)` beside `fontSize: Type.micro` reports as well, and only a shape the rule cannot read (a ternary, a template literal, a spread) goes uncompared. A hand-written literal needs an `oxlint-disable-next-line` citing why (a locked mockup ratio, a fixed-track geometry constant, an SVG style with no `lineHeight` field), not a silent rewrite. The one sanctioned non-`lineHeightFor` pairing is the verbatim `FIELD_MESSAGE_TEXT_LINE_HEIGHT` identifier (`field_message_rail.tsx`) — an aliased or re-exported reference still fails the build, by design; revert the alias instead of wrapping it.
- **Numbers:** comma-grouped — `122,300`. Every money string comes from a formatter in `src/utils/format_amount.ts` — never a hand-rolled one, hoisted or inline (M1). Rates go through `formatRateDisplayMagnitude`: 2dp (`EXCHANGE_RATE_DECIMALS`) is the escalation floor, not a fixed precision — it climbs to an 8dp ceiling until a nonzero digit shows, returning `{ text, printsAsZero }`. `formatExchangeRate`/`formatExchangeRateSentence` wrap it for the pill and sentence forms; the currency settings hero and the transaction row's rate chip call it directly for the bare text. No `rate.toFixed(2)`. Decimal counts are `.claude/rules/review.md` item 3's call. Money *colour* is `.claude/rules/review.md` item 3 too — that file's glob is the one that reaches the `.ts` resolvers this rule governs, not just the `.tsx` screens.
- **CTA:** `Size.ctaHeight` (52) · `Radius.cta` (13) · gold on midnight-blue text. Redesigned screens (onboarding, so far) render it flat `--accent` via `Button`'s `flat` prop — that path sets `Radius.cta` alone and takes HeroUI's `md` height (48), not `Size.ctaHeight` (`button.tsx:53-65`); legacy screens keep the two-stop gradient until the redesign reaches them (spec.md § Known design/codebase disagreements, item 1, update 2026-09-01). `EmptyState`'s CTA is flat on every caller, all nine screens, not per-screen opt-in (MA-025, ruling 2026-09-07).
